...
this.formatParserError = function (input, e) {
  ...
  if (!(null != e.found)) return "Syntax error on line " + e.line + ", column " + realColumn + ": unexpected end of input";
...
this.pointToErrorLocation = pointToErrorLocation = function (source, line, column, numLinesOfContext) {
  ...
  if (null == numLinesOfContext) numLinesOfContext = 3;
  lines = source.split("\n");
  ...
  return [].slice
    .call(preLines)
    .concat(["" + colourise("red", Array(padSize + 1).join("^")) + " : " + Array(column).join(" ") + colourise("red", "^")], [].slice.call(postLines))
    .join("\n");
};
...
envEnrichments_ = function (inScope) {
  var possibilities;
  if (null == inScope) inScope = [];
  possibilities = nub(
    function () {
      switch (false) {
        case !this["instanceof"](CS.AssignOp):
          return concat([beingDeclared(this.assignee), envEnrichments(this.expression)]);
        case !this["instanceof"](CS.Class):
          return concat([beingDeclared(this.nameAssignee), envEnrichments(this.parent)]);
        ...
        case !this["instanceof"](CS.Try):
          return concat([beingDeclared(this.catchAssignee), envEnrichments(this.body), envEnrichments(this.catchBody), envEnrichments(this.finallyBody)]);
