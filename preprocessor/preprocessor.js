...
StringScanner = require("./stringscanner/StringScanner");
this.Preprocessor = Preprocessor = (function () {
  ...
  Preprocessor.prototype.observe = function (c) {
    ...
    switch (c) {
      ...
      case "heregexp-{":
        ...
        break;
      case DEDENT:
        if (!(top === INDENT)) this.err(c);
        this.indents.pop();
        this.context.pop();
        break;
      case "\n":
        if (!(top === "#" || top === "heregexp-#")) this.err(c);
        this.context.pop();
        break;
      case "]":
        if (!(top === "[" || top === "regexp-[" || top === "heregexp-[")) this.err(c);
        this.context.pop();
        break;
      case ")":
        if (!(top === "(" || top === "regexp-(" || top === "heregexp-(")) this.err(c);
        ...
        break;
      case "}":
        if (!(top === "#{" || top === "{" || top === "regexp-{" || top === "heregexp-{")) this.err(c);
        this.context.pop();
        break;
      case "end-\\":
        if (!(top === "\\")) this.err(c);
  ...
  Preprocessor.prototype.p = function (s) {
    if (null != s) this.preprocessed = "" + this.preprocessed + s;
    return s;
  };
  ...
  Preprocessor.prototype.process = function (input) {
    var tok;
    if (this.options.literate) input = input.replace(/^( {0,3}\S)/gm, "    #$1");
    ...
    while (!this.ss.eos()) {
      switch (this.peek()) {
        ...
        case "(":
          ...
          break;
        case "\\":
          if (this.scan(/[\s\S]/)) this.observe("end-\\");
          break;
        case '"""':
          this.scan(/(?:[^"#\\]+|""?(?!")|#(?!{)|\\.)+/);
          this.ss.scan(/\\\n/);
        ...
        case '"':
          this.scan(/(?:[^"#\\]+|#(?!{)|\\.)+/);
          this.ss.scan(/\\\n/);
          if ((tok = this.scan(/#{|"/))) this.observe(tok);
          break;
        case "'''":
          this.scan(/(?:[^'\\]+|''?(?!')|\\.)+/);
          this.ss.scan(/\\\n/);
